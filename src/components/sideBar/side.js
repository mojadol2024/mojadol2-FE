...
function Sidebar({ onToggle }) {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      alert(
        '로그아웃 실패: ' +
          (error.response?.data?.message ?? '네트워크 오류 또는 알 수 없는 오류가 발생했습니다.')
      );
      localStorage.removeItem('accessToken');
      window.location.href = '/homepage';
    }
  ...
  return (
    <div className={`sidebar-wrapper ${isOpen ? '' : 'sidebar-collapsed'}`}>
      ...
      <ul className="sidebar-menu sidebar-top-menu">
        <li
          className={`sidebar-menu-item ${currentPath === '/InterviewMain' ? 'active' : ''}`}
        ...
        <li
          className={`sidebar-menu-item ${currentPath === '/SpellingCorrection' ? 'active' : ''}`}
        ...
        <li
          className={`sidebar-menu-item ${currentPath === '/Payment' ? 'active' : ''}`}
      ...
      <ul className="sidebar-menu sidebar-bottom-menu">
        <li
          className={`sidebar-menu-item ${currentPath === '/mypage' ? 'active' : ''}`}
